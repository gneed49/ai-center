...
| Flow déterministe PostgreSQL   | `Vérifié`                    | Base neuve, rôle RLS, `mvp_flow` et navigateur full-stack passent.                          | Reproduction CI distante.                      |
...
| ----------------------------------------------------------- | --------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| `python3 scripts/check-markdown-links.py docs specs`        | `Vérifié` — registre de liens locaux valide.                                | Les URLs distantes ne sont pas contactées.                                  |
...
| intégrations PostgreSQL                                     | `Vérifié` — 6/6 sur quatre binaires.                                        | Stack locale isolée, pas CI distante.                                       |
| pgtap + vérificateur runtime                                | `Vérifié` — 32/32 et rôle dédié conforme.                                   | Stack locale isolée.                                                        |
| Playwright API mockée                                       | `Vérifié` — 19/19 par projet, 57/57 total.                                  | Contrats HTTP simulés.                                                      |
| Playwright full-stack Chromium `1440×900`                   | `Vérifié` — 1/1, reload, zéro pageerror/console.error/requestfailed.        | Happy path unique ; erreurs full-stack certifiées aux couches DB/faux HTTP. |
| navigateur intégré                                          | `Vérifié` — Center, projet, handoff et provenance ContextPack inspectés.    | Contrôle exploratoire desktop.                                              |
| `./scripts/ci-desktop.sh desktop`                           | `Vérifié` — build web puis ELF Tauri Linux release.                         | Smoke métier natif interactif encore ouvert.                                |
| backup/restore                                              | `Vérifié` — 39 tables, 96 policies, hash logique identique, cleanup `0`.    | Environnement PostgreSQL local isolé.                                       |
| magic link Supabase                                         | `Vérifié` — `200/200/403/403/401`, cleanup utilisateur et membership.       | Pas de rotation JWKS réelle ni environnement HTTPS.                         |
...
| G1 Reproductibilité | ACP-070, 071, 076, 077 | CI desktop                       | logs et builds             | `Vérifié` local, CI distante ouverte    |
...
| G8 Certification    | ACP-030 à 077          | CI/pré-alpha complète            | rapport de certification   | `Présent mais non reproduit` à distance |
...
python3 scripts/check-markdown-links.py docs specs
