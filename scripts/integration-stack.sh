...
case "${1:-help}" in
  ...
  run)
    shift
    if (( $# == 0 )); then set -- integration tls-smoke backup-restore auth-smoke real-e2e; fi
    for integration_phase in "$@"; do
      case "${integration_phase}" in
        integration | tls-smoke | oci-api-smoke | backup-restore | auth-smoke | real-e2e | native-e2e) ;;
...
PY
  ...
  *)
    printf '%s\n' \
      'Usage: ./scripts/integration-stack.sh prepare|run [integration tls-smoke oci-api-smoke backup-restore auth-smoke real-e2e native-e2e]|stop' \
