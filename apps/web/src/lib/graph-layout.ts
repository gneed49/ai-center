...
export function internalSourceLink(
  ...
  const route = new RegExp(
    `^/(?:artifacts/${uuid}\\?version=${uuid}(?:&ticket=(?:[0-9]|[12][0-9]))?|projects/${uuid}/(?:(?:deliverables|insights|sessions)/${uuid}|sources/(?:knowledge|context_pack|task|artifact)/${uuid}|code\\?observation=${uuid}&file=${uuid}))$`,
  );
